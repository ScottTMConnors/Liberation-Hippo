...
_unitIndex = 0;
for [{_j = 0}, { (count _buildingPosArray > 0)}, {I(_j)}] do {
    scopeName "for";
                    ...
                    if !(lineIntersects [_housePos, _checkPos, objNull, objNull]) then {
                        ...
                        if (!(_isRoof) || {((_isRoof) && {(_putOnRoof)})}) then {
                            ...
                            if (!(_isRoof) || {_edge}) then {
                                _classname = selectRandom _units;
                                _unit = _group createUnit [_classname, _housePos, [], 0, "NONE"];
                                _unit doWatch ([_housePos, CHECK_DISTANCE, (90 - _i), (_housePos select 2) - (getTerrainHeightASL _housePos)] call _Zen_ExtendPosition);
                                _returnUnits pushBack _unit;
                                _unit disableAI "TARGET";
                                if (_doMove) then {
                                    _unit doMove ASLToATL ([(_housePos select 0), (_housePos select 1), (_housePos select 2) - EYE_HEIGHT]);
                                } else {
                                    _unit setPosASL [(_housePos select 0), (_housePos select 1), (_housePos select 2) - EYE_HEIGHT];
                                    _unit setDir _i;
