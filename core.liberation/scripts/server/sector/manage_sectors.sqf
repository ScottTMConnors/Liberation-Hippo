...
sectorNeighbors = [];
publicVariable "sectorNeighbors";
connectMarkers = [];
publicVariable "connectMarkers";
fobMarks = [];
fobSects = [];
...
calc_sec_sel = {
    _sectors_positions = [];
    {
        _sectors_positions pushBack [getMarkerPos _x, _x];
    } forEach sectors_allSectors;

    _sectors_positions append fobMarks;

    _findNearestNeighbors = {
        params ["_pos", "_allPositions"];
        private _nearestNeighbors = [];
        {
            if !(_x#0 isEqualTo _pos) then {
                _pos1 = _x#0;
                private _isBlocked = false;
                {
                    if (!(_x#0 isEqualTo _pos1) && !(_x#0 isEqualTo _pos)) then {
                        if (((_pos distance _x#0) < (_pos distance _pos1)) && ((_pos1 distance _x#0) < (_pos distance _pos1))) then {
                            _isBlocked = true;
                        };
                    };
                } forEach _allPositions;
                if (!_isBlocked) then {
                    _nearestNeighbors pushBack _x;
                };
            };
        } forEach _allPositions;
        _nearestNeighbors
    ...
    _sectorNeighbors = [];
    {
        ...
        private _neighbors = [_pos, _sectors_positions] call _findNearestNeighbors;
        private _neighborNames = _neighbors apply {_x select 1};
        _sectorNeighbors pushBack [_name, _neighborNames];
        {
            _connections pushBack [_pos, _x select 0];
        } forEach _neighbors;
    } forEach _sectors_positions;
    sectorNeighbors = createHashMapFromArray _sectorNeighbors;
    publicVariable "sectorNeighbors";
    
    {
        deleteMarker _x;
    } forEach connectMarkers;
    connectMarkers = [];
    {
        _startPos = _x select 0;
        _endPos = _x select 1;
        _direction = _startPos getDir _endPos;
        _distance = (_startPos distance _endPos)/2;
        _mpos = _startPos getPos [_distance, _direction];
        _markerName = format ["line_%1_%2", _mpos select 0, _mpos select 1];
        _marker = createMarker [_markerName, _mpos];
        connectMarkers pushBack _marker;
...
addMissionEventHandler ["MapSingleClick", {
	...
	if (active_sectors isEqualTo []) then {
        ...
        _available = [];
        _fsects = blufor_sectors + fobSects;
        _nearSectorsActivated = 100;
        if (!(_fsects isEqualTo [])) then {
            {_available append ((sectorNeighbors get _x) select {_x in opfor_sectors})} forEach _fsects;
        } else {
            //_available = opfor_sectors;
        };
		{
			_distance = (getMarkerPos _x) distance _pos;
		...
		if (!(_closestMarker isEqualTo "")) then {
			{
				if (((getMarkerPos _x) distance (getMarkerPos _closestMarker)) < _nearSectorsActivated) then {
